...
repo --name="repo0" --baseurl=http://mirror.centos.org/centos/7/os/x86_64
repo --name="repo1" --baseurl=http://mirror.centos.org/centos/7/updates/x86_64
repo --name="repo2" --baseurl=http://dl.fedoraproject.org/pub/epel/7/x86_64/
...
# fix the cloud-init config, so its for centos and not for fedora
sed -i 's/name: fedora/name: centos/g' /etc/cloud/cloud.cfg
sed -i 's/gecos: Fedora Cloud User/gecos: CentOS Cloud User/g' /etc/cloud/cloud.cfg
